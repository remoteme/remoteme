...
def ask(question):
    ...
    while (answer != 'N') & (answer != 'Y'):
        answer = input(
            question + bcolors.OKGREEN + " Y" + bcolors.ENDC + "es " + bcolors.FAIL + "N" + bcolors.ENDC + "o type Y or N and press enter\n")
        answer = answer.upper()
    return answer == 'Y'
...
def updateConf(token, name, deviceId):
    with open("conf.json", "r") as jsonFile:
        data = json.load(jsonFile)

    if token == None and data['server'].get('token', None) != None:
        if not ask(
                "there is already provided token [{}] would You like to replace it ?".format(data['server']['token'])):
    ...
    if token != None:
        ...
        if userName == None:
            print(bcolors.FAIL +"Provided token not fround at app.remoteme.org" + bcolors.ENDC)
    else:
        userName = None
    ...
    # ------------------ NAME ---------------------------------------
    if name == None and data['server'].get('name', None) != None:
        if not ask("there is already provided name of device [{}] would You like to replace it ?".format(
                data['server']['name'])):
...
# ------------------- ID -----------------------------------------
    ...
    if deviceId == None and data['server'].get('deviceId', None) != None:
        if not ask("there is already provided deviceId of device [{}] would You like to replace it ?".format(
                data['server']['deviceId'])):
...
if yesToAll or ask(
        "Would You like to install  adafruit-pca9685 - it's with playting with PWM servo mechanism device ?"):
    isntallAdafruitPWM()

if token != None or deviceId != None or name != None or ask(
        "Would You like to update your configuration ? - its needed for proper connecting remoteme to server"):
    updateConf(token, name, deviceId)
